...
final class ScrollerViewModel: ObservableObject {
    ...
    private func generateFeedItem() -> FeedItem {
        ...
        let unlockedOfRarity = allOfRarity.filter { unlockedSkinIDs.contains($0.id) || commonCatalog.contains(where: { $0.id == $0.id }) }
    ...
    private func attemptUnlock(skin: Skin) {
        ...
        // Inject real generated set for rare unlocks
        if skin.rarity == .rare {
            // Create a seed
            let seed = UInt64(Date().timeIntervalSince1970)

            // Create 10 skins: 6 common, 3 rare, 1 special with varying hues
            var newSkins: [Skin] = []

            func colorFromHue(_ hue: Double) -> Color {
                Color(hue: hue.truncatingRemainder(dividingBy: 1.0), saturation: 0.7, brightness: 0.9)
            }
            ...
            for i in 0..<6 {
                let skin = Skin(
                    id: UUID(),
                    name: "Generated Common \(i+1)",
                    rarity: .common,
                    style: .solid(colorFromHue(Double(i)/6.0))
                )
                newSkins.append(skin)
            }
            // 3 rares
            for i in 0..<3 {
                let skin = Skin(
                    id: UUID(),
                    name: "Generated Rare \(i+1)",
                    rarity: .rare,
                    style: .gradient([colorFromHue(Double(i)/3.0), colorFromHue(Double(i)/3.0 + 0.1)])
                )
                newSkins.append(skin)
            }
            // 1 special
            let specialSkin = Skin(
                id: UUID(),
                name: "Generated Special 1",
                rarity: .special,
                style: .stripes(.black, .white)
            )
            newSkins.append(specialSkin)
...
struct ContentView: View {
    ...
    @State private var shimmerTriggerID: UUID? = nil
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack {
                ...
                .onReceive(NotificationCenter.default.publisher(for: .didInjectGeneratedSet)) { note in
                    shimmerTriggerID = note.object as? UUID
                    DispatchQueue.main.asyncAfter(deadline: .now() + 1.2) { shimmerTriggerID = nil }
                }
                .ignoresSafeArea()
                .simultaneousGesture(
                    ...
                    HStack {
                        ...
                        Button { showInventory = true } label: {
                            ZStack {
                                ...
                                if shimmerTriggerID != nil {
                                    InventoryPill(unlockedCount: vm.unlockedSkins.count)
                                        .subtleShimmer(duration: 1.0)
                                }
                            }
                        }
                        .padding(.trailing, 14)
